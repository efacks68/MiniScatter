...
if os.path.isfile(mapCsvPWD+name+".csv"):
    ...
    for i in range(len(rXAmps)):
        for j in range(len(rYAmps)):
            ...
            rasterBeamFile, beamXAngle, beamYAngle = runARasterMaker(args.energy,args.Nb,args.nP,args.rX,args.rY,args.edgeRaster,Twiss,rXAmps[i],rYAmps[j],csvPWD,options)
            #Send raster beam file to runPBW which simulates with MiniScatter or opens already run data. Full PBW model
            Jmaxes[j][i], pOutsideBoxes[j][i], dispY,dispX,rValue = runPBW(args.energy,rasterBeamFile,args.t,beamXAngle,beamYAngle,args.savePics,Twiss,rXAmps[i],rYAmps[j],options)
