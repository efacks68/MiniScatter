...
def betaPOutFit(args,Twiss,paths,origBX,origBY,beamFile):
...
#    from matplotlib.pyplot import scatter,savefig,close,plot,xlim,ylim,text,title,xlabel,ylabel,tight_layout,xticks
    ...
    #print(mean(betas),mean(pOuts))

    #scatter(betas,pOuts)
    
    #model = LinearRegression(betas,pOuts)
    ...
    plt.tight_layout()
    print(paths['statsPWD']+args.twissFile+"_betaVpOut"+axis+".png")
    plt.savefig(paths['statsPWD']+args.twissFile+"_betaVpOut"+axis+".png")
