...
title1=r"Distribution of $\beta_x$ Values for "+re.search("(([0-9]*)+(?=Pct))",name)[1]+"% Jitter"
#print(title1)
...
title2=r"Distribution of $\beta_y$ Values for "+re.search("(([0-9]*)+(?=Pct))",name)[1]+"% Jitter"
plt.setp(s2,title=title1,xlabel=r"$\beta$ [m]",ylabel="Counts [a.u.]")
plt.tight_layout()
plt.savefig(name+"Spread.png",bbox_inches='tight')
print(name+"Spread.png")
