...
def calcEq8(thetasq,Twiss,thick,beta_rel,gamma_rel):
  ...
  e8beta = Twiss[2] * Twiss[0]*m / (Twiss[2] + e8dgem)
  e8gemt = Twiss[2] + e8dgem
  #e8nemt = e8nemt/(beta_rel*gamma_rel)

  return [e8beta,e8alph,e8gemt]#e8nemt,
...
def calcEq16(thetasq,Twiss,thick,beta_rel,gamma_rel):
  ...
  e16dgem = 0.5 * thetasq * thetasq * (Twiss[0]*m + thick*mm * Twiss[1] + thick*mm*thick*mm/3 * gamma0) #[m*rad^2]
  e16alph = (Twiss[2] * Twiss[1] - thick*mm * 0.5 * thetasq) / (Twiss[2] + e16dgem)
  e16beta = (Twiss[2] * Twiss[0]*m + thick*mm * thick*mm / 3 * thetasq) / (Twiss[2] + e16dgem)
  e16gemt = Twiss[2] + e16dgem
  #e16nemt = e16nemt*(beta_rel*gamma_rel)

  return [e16beta,e16alph,e16gemt]

def getMoments(Twiss):
  from numpy import sqrt as sq
  mm=1e-3
  #[beta,alpha,gemt]
  sigma = sq(Twiss[0]*Twiss[2]) /mm #sigma rms = beam size = sqrt(beta*epsG)
  mu = Twiss[0]/Twiss[2] /mm #posVar = beta/epsG 
  sigmapx = sq(Twiss[2]*(1+Twiss[1]*Twiss[1])/Twiss[0]) #mrad actually
  #print(mu,sigma,sigmapx)
  return [mu,sigma,sigmapx]

def plotTwissFit(xs,pxs,savename,mat,titledescr,axis,thick,thetasq,beta_rel,gamma_rel):
  ...
  #print("PDF")
  print("Calculated  sigmaTwx: {:.2f}mm, sigmaTwX': {:.2f}mrad".format(Mcal[1],Mcal[2]/mm))
  print("Equation 8  sigmaTwx: {:.2f}mm, sigmaTwX': {:.2f}mrad".format(Me8[1],Me8[2]/mm))
  print("Equation 16 sigmaTwx: {:.2f}mm, sigmaTwX': {:.2f}mrad".format(Me16[1],Me16[2]/mm))
  ...
  sigmatextx +="\nFit = "+"{:.2f}".format(Mcal[1])+"mm"
  sigmatextx +="\n"+PDFlabele8+" = "+"{:.2f}".format(Me8[1])+"mm"
  sigmatextx +="\n"+PDFlabele16+" = "+"{:.2f}".format(Me16[1])+"mm"
  sigmatextpx +="\nDistribution = "+"{:.2f}".format(sigmapx/mm)+"mrad"
  sigmatextpx +="\nFit = "+"{:.2f}".format(Mcal[2]/mm)+"mrad"
  sigmatextpx +="\n"+PDFlabele8+" = "+"{:.2f}".format(Me8[2]/mm)+"mrad"
  sigmatextpx +="\n"+PDFlabele16+" = "+"{:.2f}".format(Me16[2]/mm)+"mrad"
